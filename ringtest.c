...
#define a_assert(o) 
#define balloc(a, b) malloc(b)
#define bfree(a, b) free(b)
typedef char char_t;
...
{
	ringq_t data;
	if (ringqOpen(&data, 100, 1024))
	{
		printf("Cannot init ring data\n");
		return 0;
	}

	int rc = ringqPutStr(&data, "TRINH VAN LINH");
	if (rc == 0)
	{
		printf("Cannot put string into ringq\n");
	}
	else
	{
		printf("Put string into ringq successfully\n");
	}
	...
	do
	{
		...
		rc = ringqGetBlk(&data, str, sizeof str - 1);
		printf("rc: %d: %s\n", rc, str);
	} while (rc);

	rc = ringqPutStr(&data, "FAILED TO FAILED");
	if (rc == 0)
	{
		printf("Cannot put string into ringq\n");
	}
